...
7. **用户可见改动？** **先读设计系统 `docs/design/nomi-design-system.md`（token/组件/规范）再画**，再出样张；token-only（禁非 token 的 px/hex）；渐进展开 / 防遮挡 / 空间极简。（→ R8）
8. **准备 push？** 五门全过了吗？（→ R11）
9. **要报完成？** 和样张对账过了吗 + 体感走查过了吗？（→ P3）

## 规则索引（详见下方「详细规则 R1–R14」）
